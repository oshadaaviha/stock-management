...
async function testPurchaseInsert() {
  ...
  try {
    ...
    const [res1]: any = await conn.query(
      ...
      [testData.refNo, testData.supplier, testData.batch_number, testData.sub_total, testData.total]
    );
    ...
    await conn.query(
      ...
      [itemData.purchase_id, itemData.product_id, itemData.mfg_date, itemData.exp_date, itemData.pack_size, itemData.price, itemData.cost, itemData.qty, itemData.line_total]
    );
