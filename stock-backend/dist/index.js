...
app.post("/api/auth/login", async (req, res) => {
    ...
    try {
        ...
        const [rows] = await db_1.pool.query("SELECT id, username, name, email, password, role FROM users WHERE username=? AND status='Active'", [d.username]);
        if (!rows.length) {
            return res.status(401).json({ ok: false, error: 'Invalid username or password' });
        }
        const user = rows[0];
        const validPassword = await bcryptjs_1.default.compare(d.password, user.password);
        if (!validPassword) {
            return res.status(401).json({ ok: false, error: 'Invalid username or password' });
        }
...
// Get current user
app.get("/api/auth/me", auth_1.authenticate, (req, res) => {
    res.json({ ok: true, user: req.user });
});
// Change password
app.post("/api/auth/change-password", auth_1.authenticate, async (req, res) => {
    const schema = zod_1.z.object({
    ...
    try {
        const d = schema.parse(req.body);
        const [rows] = await db_1.pool.query("SELECT password FROM users WHERE id=?", [req.user.id]);
        if (!rows.length) {
            return res.status(404).json({ ok: false, error: 'User not found' });
        }
        const validPassword = await bcryptjs_1.default.compare(d.oldPassword, rows[0].password);
        if (!validPassword) {
            return res.status(401).json({ ok: false, error: 'Current password is incorrect' });
        }
        const hashedPassword = await bcryptjs_1.default.hash(d.newPassword, 10);
        await db_1.pool.query("UPDATE users SET password=? WHERE id=?", [hashedPassword, req.user.id]);
        res.json({ ok: true, message: 'Password changed successfully' });
    }
    catch (err) {
        console.error("Change password error:", err);
        res.status(400).json({ ok: false, error: err.message || 'Failed to change password' });
    }
});
/* ---------- User Management (Admin Only) ---------- */
// Get all users
app.get("/api/users", auth_1.authenticate, (0, auth_1.authorize)('Admin'), async (_req, res) => {
    const [rows] = await db_1.pool.query("SELECT id, username, name, email, role, status, created_at FROM users ORDER BY created_at DESC");
    res.json(rows);
});
// Create user
app.post("/api/users", auth_1.authenticate, (0, auth_1.authorize)('Admin'), async (req, res) => {
    const schema = zod_1.z.object({
        username: zod_1.z.string().min(3),
        name: zod_1.z.string().min(1),
        email: zod_1.z.string().email(),
        password: zod_1.z.string().min(6),
        role: zod_1.z.enum(['Admin', 'Finance', 'Reporter'])
    });
    try {
        const d = schema.parse(req.body);
        const hashedPassword = await bcryptjs_1.default.hash(d.password, 10);
        const [r] = await db_1.pool.query("INSERT INTO users (username, name, email, password, role, status) VALUES (?,?,?,?,?,'Active')", [d.username, d.name, d.email, hashedPassword, d.role]);
...
// Update user
app.put("/api/users/:id", auth_1.authenticate, (0, auth_1.authorize)('Admin'), async (req, res) => {
    ...
    try {
        const d = schema.parse(req.body);
        await db_1.pool.query("UPDATE users SET username=?, name=?, email=?, role=?, status=? WHERE id=?", [d.username, d.name, d.email, d.role, d.status, id]);
        res.json({ ok: true });
    }
    catch (err) {
...
// Reset user password
app.post("/api/users/:id/reset-password", auth_1.authenticate, (0, auth_1.authorize)('Admin'), async (req, res) => {
    const id = Number(req.params.id);
    const schema = zod_1.z.object({
        newPassword: zod_1.z.string().min(6)
    });
    try {
        const d = schema.parse(req.body);
        const hashedPassword = await bcryptjs_1.default.hash(d.newPassword, 10);
        await db_1.pool.query("UPDATE users SET password=? WHERE id=?", [hashedPassword, id]);
...
// Delete user
app.delete("/api/users/:id", auth_1.authenticate, (0, auth_1.authorize)('Admin'), async (req, res) => {
    const id = Number(req.params.id);
...
app.post("/api/purchases", async (req, res) => {
    ...
    try {
        ...
        const [res1] = await conn.query("INSERT INTO purchases (ref_no, supplier, batch_number, sub_total, total) VALUES (?,?,?,?,?)", [d.refNo, d.supplier ?? null, d.batch_number, sub, sub]);
        const pid = res1.insertId;
        for (const it of d.items) {
            await conn.query("INSERT INTO purchase_items (purchase_id, product_id, mfg_date, exp_date, pack_size, price, cost, qty, line_total) VALUES (?,?,?,?,?,?,?,?,?)", [pid, it.productId, it.mfg_date, it.exp_date, it.pack_size, it.price, it.cost, it.qty, it.qty * it.cost]);
            await conn.query("UPDATE products SET qty = qty + ?, cost=?, price=? WHERE id=?", [it.qty, it.cost, it.price, it.productId]);
        }
        await conn.commit();
...
app.post("/api/sales", async (req, res) => {
    const schema = zod_1.z.object({
        customer: zod_1.z.object({
            ...
            customer_vat: zod_1.z.string().optional(),
            sales_rep_id: zod_1.z.number().int().optional()
        }),
        items: zod_1.z.array(zod_1.z.object({
            sku: zod_1.z.string(),
            qty: zod_1.z.number().int().nonnegative(), // number of packs being sold (0 allowed for write-offs)
            price: zod_1.z.number().nonnegative(), // unit price (per 1 unit)
            discount: zod_1.z.number().nonnegative().default(0), // unit discount
            packSize: zod_1.z.string().optional(), // e.g. "10" or "6x10"
            batchNo: zod_1.z.string().optional(), // item-level batch selection
        })).min(1),
        ...
        batchNo: zod_1.z.string().optional(), // legacy / fallback
        paymentType: zod_1.z.string().optional(),
        routeRepCode: zod_1.z.string().optional(),
        salesRepName: zod_1.z.string().optional(),
    });
    const d = schema.parse(req.body);
    const invNo = await (0, utils_1.nextInvoiceNo)(d.invoiceDate);
    const conn = await db_1.pool.getConnection();
    try {
        await conn.beginTransaction();
        ...
        const [existingCustomers] = await conn.query("SELECT id, customer_code FROM customers WHERE name=? AND is_system=TRUE LIMIT 1", [d.customer.name]);
        if (existingCustomers.length > 0) {
            ...
            await conn.query("UPDATE customers SET customer_address=?, customer_vat=?, sales_rep_id=? WHERE id=?", [d.customer.customer_address ?? null, d.customer.customer_vat ?? null, d.customer.sales_rep_id ?? null, customerId]);
        }
        else {
            // Create walk-in customer (mark as auto-created with is_system=FALSE)
            const [cRes] = await conn.query("INSERT INTO customers (name, phone, customer_address, customer_vat, sales_rep_id, is_system) VALUES (?,?,?,?,?,?)", [d.customer.name, d.customer.phone ?? null, d.customer.customer_address ?? null, d.customer.customer_vat ?? null, d.customer.sales_rep_id ?? null, false]);
        ...
        for (const it of d.items) {
            ...
            if (it.qty > 0) {
                ...
                if (it.batchNo && it.batchNo.startsWith('PI#')) {
                    ...
                    const [piRows] = await conn.query("SELECT id, qty FROM purchase_items WHERE id=?", [piId]);
                    if (!piRows.length || piRows[0].qty < it.qty)
                    ...
                    const [piRows] = await conn.query("SELECT id, qty, exp_date FROM purchase_items WHERE product_id = (SELECT id FROM products WHERE sku=?) AND qty > 0 ORDER BY exp_date ASC, id ASC", [it.sku]);
                ...
                if (it.batchNo && !it.batchNo.startsWith('PI#')) {
                    const [batchRows] = await conn.query("SELECT id, quantity FROM stock_batches WHERE batch_number=? AND product_id=(SELECT id FROM products WHERE sku=?) LIMIT 1", [it.batchNo, it.sku]);
                    if (!batchRows.length || batchRows[0].quantity < it.qty)
                        throw new Error("INSUFFICIENT_BATCH_STOCK");
                    await conn.query("UPDATE stock_batches SET quantity = quantity - ? WHERE id=?", [it.qty, batchRows[0].id]);
                }
            }
            // Insert sales row (always, even if qty=0)
            const [prodRows] = await conn.query("SELECT id, name FROM products WHERE sku=?", [it.sku]);
            const productName = prodRows.length ? prodRows[0].name : it.sku;
            const lineTotal = (it.qty * packUnits) * (it.price - it.discount);
            await conn.query(`INSERT INTO sales (invoice_no, invoice_date, customer_id, sales_rep_id, payment_type, batch_no, route_rep_code, sales_rep_name, customer_address, customer_vat, sku, name, pack_size, qty, price, sub_total, tax, discount, total) 
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [
                invNo,
...
app.get("/api/sales/:id/invoice", async (req, res) => {
    ...
    const [sRows] = await db_1.pool.query("SELECT * FROM sales WHERE invoice_no=?", [invoiceNo]);
    if (!sRows.length)
        return res.status(404).send("Not found");
    const firstRow = sRows[0];
    const [cRows] = await db_1.pool.query("SELECT * FROM customers WHERE id=?", [firstRow.customer_id]);
    const customer = cRows[0];
    // Format items from sales rows
    const items = sRows.map((row) => ({
        name: row.name,
    ...
    const itemDiscountTotal = items.reduce((sum, item) => sum + (item.qty * item.discount), 0);
    const grossTotal = items.reduce((sum, item) => sum + item.line_total, 0);
    ...
    const html = format === 'dot'
        ? (0, utils_1.invoiceDotMatrixHtml)({
...
app.get("/api/reports/stock", async (_req, res) => {
    const [rows] = await db_1.pool.query("SELECT id,sku,name,qty,cost,price,(qty*cost) AS stock_value FROM products ORDER BY name");
    res.json(rows);
});
/* ---------- Customers ---------- */
// list
app.get("/api/customers", async (_req, res) => {
    const [rows] = await db_1.pool.query("SELECT * FROM customers ORDER BY created_at DESC");
    res.json(rows);
});
// create
app.post("/api/customers", async (req, res) => {
    const schema = zod_1.z.object({
        customer_code: zod_1.z.string().optional().nullable(),
        name: zod_1.z.string().min(1),
        phone: zod_1.z.string().optional().nullable(),
        email: zod_1.z.string().optional().nullable(),
        customer_address: zod_1.z.string().optional().nullable(),
        customer_vat: zod_1.z.string().optional().nullable(),
        route: zod_1.z.string().optional().nullable(),
        sales_rep_id: zod_1.z.number().optional().nullable()
    });
    const d = schema.parse(req.body);
    const [r] = await db_1.pool.query("INSERT INTO customers (customer_code, name, phone, email, customer_address, customer_vat, route, sales_rep_id, is_system) VALUES (?,?,?,?,?,?,?,?,?)", [d.customer_code ?? null, d.name, d.phone ?? null, d.email ?? null, d.customer_address ?? null, d.customer_vat ?? null, d.route ?? null, d.sales_rep_id ?? null, true]);
...
app.put("/api/customers/:id", async (req, res) => {
    ...
    const d = schema.parse(req.body);
    await db_1.pool.query("UPDATE customers SET customer_code=?, name=?, phone=?, email=?, customer_address=?, customer_vat=?, route=?, sales_rep_id=? WHERE id=?", [d.customer_code ?? null, d.name, d.phone ?? null, d.email ?? null, d.customer_address ?? null, d.customer_vat ?? null, d.route ?? null, d.sales_rep_id ?? null, id]);
    res.json({ ok: true });
});
// delete
app.delete("/api/customers/:id", async (req, res) => {
    await db_1.pool.query("DELETE FROM customers WHERE id=?", [req.params.id]);
    res.json({ ok: true });
});
/* ---------- Suppliers ---------- */
app.get("/api/suppliers", async (req, res) => {
    try {
        const [rows] = await db_1.pool.query("SELECT id, name, phone, email, address, created_at FROM suppliers ORDER BY name ASC");
        res.json(rows);
    }
    catch (err) {
        console.error(err);
...
app.post("/api/suppliers", async (req, res) => {
    ...
    const d = schema.parse(req.body);
    try {
        const [result] = await db_1.pool.query("INSERT INTO suppliers (name, phone, email, address) VALUES (?, ?, ?, ?)", [d.name, d.phone ?? null, d.email ?? null, d.address ?? null]);
        const [rows] = await db_1.pool.query("SELECT * FROM suppliers WHERE id = ?", [result.insertId]);
...
app.put("/api/suppliers/:id", async (req, res) => {
    ...
    try {
        await db_1.pool.query("UPDATE suppliers SET name = COALESCE(?, name), phone = COALESCE(?, phone), email = COALESCE(?, email), address = COALESCE(?, address) WHERE id = ?", [d.name ?? null, d.phone ?? null, d.email ?? null, d.address ?? null, id]);
        const [rows] = await db_1.pool.query("SELECT * FROM suppliers WHERE id = ?", [id]);
        res.json(rows[0]);
    }
    catch (err) {
        console.error(err);
