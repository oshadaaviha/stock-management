...
exports.invoiceDotMatrixHtml = invoiceDotMatrixHtml;
exports.invoiceOverlayHtml = invoiceOverlayHtml;
...
async function nextInvoiceNo(invoiceDate) {
    ...
    const [rows] = await db_1.pool.query("SELECT invoice_no FROM sales WHERE invoice_no LIKE ? OR invoice_no LIKE ?", [pNew + '%', pOld + '%']);
    let maxSeq = 0;
    for (const r of rows) {
...
function invoiceDotMatrixHtml(inv) {
    ...
    function pad(str, len, right = true) {
        ...
        if (s.length === len)
            return s;
        if (s.length > len)
            return right ? s.slice(0, len) : s.slice(-len);
    ...
    lines.push(pad('DESCRIPTION', COL_NAME) +
    ...
    for (const it of inv.items) {
        ...
        lines.push(pad(it.name, COL_NAME) +
...
function invoiceOverlayHtml(inv) {
    const mm = (n) => `${n}mm`;
    ...
    function text(x, y, value, align = 'left') {
    ...
    let t = Y.totalsTop;
    html += text(totalsX, t, fmt(inv.sub_total), 'right');
    t += 5.5;
    html += text(totalsX, t, 'VAT(18%)' + fmt(inv.tax), 'right');
    t += 5.5;
    html += text(totalsX, t, fmt(inv.discount), 'right');
    t += 5.5;
